...
(function() {
  ...
  convertBtn.addEventListener('click', async () => {
    ...
    progressText.innerText = 'Extracting content from Word doc...';

    try {
      const result = await mammoth.convertToHtml({ arrayBuffer: fileBuffer });
      const rawHtml = result.value; 

      if (!rawHtml) throw new Error("Could not extract any content from the document.");
      ...
      convertBtn.disabled = false;
      hiddenContainer.innerHTML = '';
      alert(err.message || 'Error converting Word to PDF.');
    }
  });
  ...
  downloadBtn.addEventListener('click', () => {
    ...
    link.download = currentFile.name.replace(/\.docx$/i, '') + '.pdf';
