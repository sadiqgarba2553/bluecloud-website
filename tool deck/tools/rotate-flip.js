...
(function() {
  ...
  function updatePreviews() {
    ...
    thumbs.forEach(function(t) {
      t.style.transform = 'rotate(' + rot + 'deg) scaleX(' + scaleX + ') scaleY(' + scaleY + ')';
    });
  }

  $('btn-rot-left').addEventListener('click', function() {
    rot -= 90;
    updatePreviews();
  });
  $('btn-rot-right').addEventListener('click', function() {
    rot += 90;
    updatePreviews();
  });
  ...
  $('btn-flip-v').addEventListener('click', function() {
    scaleY *= -1;
    updatePreviews();
  });
  ...
  function renderList() {
    ...
    filesData.forEach(function(item, index) {
      ...
      // apply current transform immediately to new files
      img.style.transform = 'rotate(' + rot + 'deg) scaleX(' + scaleX + ') scaleY(' + scaleY + ')';
  ...
  function processImage(file, r, sx, sy) {
  ...
  btnStart.addEventListener('click', async function() {
    ...
    try {
      for (var i = 0; i < filesData.length; i++) {
        ...
        var convertedBlob = await processImage(item.file, rot, scaleX, scaleY);
      ...
      btnDownload.download = 'processed-images.zip';
      ...
      if (filesData.length === 1) {
        ...
        btnDownload.download = 'mod-' + singleName;
