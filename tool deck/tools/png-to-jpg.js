...
(function() {
  ...
  function convertToJpg(file, quality, bg) {
    return new Promise(function(resolve, reject) {
      ...
      img.onload = function() {
        ...
        // Fill background first because JPG doesn't support transparency
        ctx.fillStyle = bg;
  ...
  btnStart.addEventListener('click', async function() {
    ...
    try {
      for (var i = 0; i < filesData.length; i++) {
        ...
        progressText.textContent = 'Converting Image ' + (i+1) + ' of ' + filesData.length;
        progressFill.style.width = Math.round((i / filesData.length) * 100) + '%';
        
        var convertedBlob = await convertToJpg(item.file, quality, bg);
        var newName = item.file.name.replace(/\.[^/.]+$/, "") + ".jpg";
      ...
      btnDownload.download = 'converted-jpgs.zip';
      
      // If single file, change download logic to just download the file
      if (filesData.length === 1) {
        var singleName = filesData[0].file.name.replace(/\.[^/.]+$/, "") + ".jpg";
